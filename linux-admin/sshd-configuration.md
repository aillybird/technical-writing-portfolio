...
## Overview
This guide will walk you through setting up an sshd server, basic SSH security, and how to create and use SSH keys. 

The default configuration values ease of use and accessibility over security. Securing your sshd server is vital as it is one of the most common entry points on Linux systems, with many automated scanning and brute force attacks targetting any sshd server found on the internet.

This guide covers basic SSH hardening and is not a complete server security checklist.

You will need a server with a publically reachable IP address and this guide assumes you will be using a Debian-based Linux operating system.
...
## Step 2: Install sshd
You can find the sshd server software in the Debian repository:
...
At this point, you will have a functioning and reasonably hardened sshd server.
...
### Copying Public Keys
Now that we have generated a public key, we can copy that key to the sshd server. Issue the following command from the remote host:
...
**DO NOT LOCK YOURSELF OUT.** Be sure to test and verify connections from your allowed hosts to make sure you can connect to your sshd server before proceeding. Keep your active session open until you verify that you can access your server after reloading.
...
**-p vs -P**: For port specification, some the `ssh` commands use `-p` (lowercase) while other commands such as `scp` use `-P` (uppercase).
